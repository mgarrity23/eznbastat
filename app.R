...
server <- function(input, output) {
  ...
  observeEvent(input$histEnter,{
    ...
    output$plot1 <- renderPlotly(
      ggplotly(ggplot(hist3) +
                 ...
                 annotate(geom="text", x=1996, y=49000, label="1995-1997 3p Line Moved in to uniform 22ft",
                          color="blue", size=3)+
  ...
  observeEvent(input$compEnter,{
    ...
    if(input$xInput==2 && input$yInput==1){
      yearly_df <- playerTable %>% 
        filter(yearSeason == input$histYear & minutesRank < input$minuteSlide & fgm != 0 & ratioASTtoTO != 0)
      
      tempplot1 <- ggplot(yearly_df, aes(x=fgm, y=netrtg)) +
        geom_point(aes(text = namePlayer)) +
        labs(x="Field Goals Made", y="Net Rating")
    }
   
    if(input$xInput==input$yInput){
      output$Error <- renderText("Stat types cannot be the same")
    }
    else{
